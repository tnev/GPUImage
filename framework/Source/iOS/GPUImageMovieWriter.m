...
- (void)processAudioBuffer:(CMSampleBufferRef)audioBuffer;
{
    ...
    if (_hasAudioTrack)
    {
        ...
        if (self.audioProcessingCallback) {
            ...
            self.audioProcessingCallback(&audioBufferList, numSamplesInBuffer, currentSampleTime);
            ...
            else if(assetWriter.status == AVAssetWriterStatusWriting)
            {
                if (![assetWriterAudioInput appendSampleBuffer:audioBuffer]) {
                    ...
                    if (assetWriter.error != nil){
                        NSLog(@"Asset Writer failed with error: %@", assetWriter.error);
                    }
                }
            }
            else
...
- (void)setHasAudioTrack:(BOOL)newValue audioSettings:(NSDictionary *)audioOutputSettings;
{
    ...
    if (_hasAudioTrack)
    {
        ...
        else if (audioOutputSettings == nil)
        {
            ...
            bool success = [sharedAudioSession setCategory:AVAudioSessionCategoryPlayAndRecord error:&error];
            if (!success) NSLog(@"Error setting AVAudioSession category! %@\n", [error localizedDescription]);
            
            //determine hardware's preferred sample rate
...
#pragma clang diagnostic pop
            ...
            success = [sharedAudioSession setActive:YES error:&error];
